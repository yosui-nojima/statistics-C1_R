# 2024年度 統計学B-II 木曜２限 2024年10月31日 10:30~12:00
# R実習
...
## 8. Rの文法
一行ずつ実行すること。\
実行する際は、実行したいコードを選択し、下記画像赤枠内の```Run```をクリックする。\
<img width="1188" alt="スクリーンショット 2023-05-17 12 32 39" src="https://github.com/yosui-nojima/statistics-C1_R_2023/assets/85273234/a1bfd5c0-6eb6-4a47-a928-281785c67759">
...
```
data <- matrix(nrow = 10, ncol = 1) #10行✕1列の空のマトリクスを作成
for (x in 1:10) {
...
```
条件文が誤っている場合、```FALSE```と判定され、下記で4行目に指定したコードが実行される。
```
if (1>3) {
  print("Correct") #if文でTRUEだった場合に実行される部分
} else {
...
```
if文とfor文を組み合わせて、繰り返し条件文とすることが可能。\
n=1,2,3,,,10が5より値として大きいかどうかをそれぞれ判定し、正しければ"Correct"を、間違っていれば"Incorrect"を2列目に格納する。

```
a <- matrix(nrow = 10, ncol = 2) #10行✕2列の空のマトリクスを作成
a[,1] <- seq(1, 10, 1)
for (x in 1:10) {
  if (as.numeric(a[x,1]) < 5) {
    a[x,2] <- "Correct" #if文でTRUEだった場合に実行される部分
  } else {
...
- matrix; マトリクスを作成ための関数
  - nrow; 行数の指定
  - ncol; 列数の指定
- as.numeric(); ()内のベクトルを数字として扱うように指定する関数
...
colnames(mean.d)[1] <- "Mean" #平均値の列にMeanという列名を指定
for (x in 1:nrow(mean.d)) { #forループ（nrowは行数を出力する関数。1:nrow(mean.d); 1からmean.dの行数までループを続ける）
...
```
- as.matrix; オブジェクトをマトリクスとして扱うための関数
- qnorm; 引数```mean =```、```sd =```で指定した平均、標準偏差の正規分布のx座標を出力
  - mean; 平均値を指定
  - sd; 標準偏差を指定
  - lower.tail; 下限値として出力するかどうか。```TRUE```で下限値として出力、```FALSE```で上限値として出力。
  - p; 有意水準αを指定
...
1. [e-Stat](https://www.e-stat.go.jp/)から任意のデータを選択し、N=100000の母集団を生成する。set.seed内の値は学籍番号の下３桁の値を入力すること。（大阪府17歳男性の身長データは対象外とする。）
2. 1.で生成した母集団から大きさn=100の標本を100回抽出し、抽出した標本それぞれについて信頼係数99%で区間推定する。
3. 2.推定した区間のうち母数が含まれている区間の数と含まれていない区間の数を確認する。

レポートには、e-Statで選択したデータの説明（どういったデータなのか）とそのデータのリンク（URL）、実行したRのコード、実行後の出力結果、実行後に出力された図を含めること。\
**※提出期限：2024年11月7日 23:59までにCLEから提出**\
**※ファイル名は「○○○○○○_20241031.pdf」とすること。○○○○○○には各自の学籍番号を入れる。**\
**※期限を過ぎるとCLEから提出できなくなります。**\
